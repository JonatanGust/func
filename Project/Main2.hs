...
--          let bxTable = boxTable'
----          state <- newIORef []
----          bxTable `eventF` (Click $ \_ -> writeIORef state [])
--
--
--
--          bxTable `onEvent` Click $ \mouse -> do
...
----                writeIORef state (ball:balls)
          ...
          runF (h2F (textF "Othello") >+ boxTable )
...
data OthelloButton = Greenb | Whiteb | Blackb
        deriving (Show,Eq)




-- =<=  stateF update [(Blackb,BtnClick)] =<= keyboardF
boxTable' = keyboardF =<= stateF update [(Whiteb,BtnClick)] =<= keyboardF -- =<=
  where
    keyboardF = tableF 4 (buttonsF )
    buttonsF = listF [op Blackb, op Greenb, op Greenb, op Greenb,
                      op Greenb, op Greenb, op Greenb, op Greenb,
                      op Greenb, op Greenb, op Greenb, op Greenb,
                      op Greenb, op Greenb, op Greenb, op Greenb]

    op o = (o,buttonF (opLabel o))

    opLabel Greenb           = "Green"
    opLabel Blackb           = "Black"


    update s (Blackb,_)      = new (Blackb,BtnClick) s
    update s (Greenb,_)      = new (Greenb,BtnClick) s
...
boxTable'' = shellF "Canvas example" $
...
polygon' btn | btn == 1      = do color (RGB 0 255 0) (fill (square 40))
                                  color (RGB 0 0 0)   (stroke $ square 40)
             | btn == 2      = do color (RGB 0 0 0)   (fill (square 40))
                                  color (RGB 0 0 0)   (stroke $ square 40)
             | otherwise     = do text (20,20) "Black"
